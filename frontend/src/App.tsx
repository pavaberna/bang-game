...
function App() {
  ...
  const copy = () => {
    if (createGameId !== null) {
      navigator.clipboard.writeText(createGameId.toString())
    }
  }
      ...
      {inGame === false ? (
        <div>
          {loginSuccess !== true ? (
            ...
            <main className="flex flex-col items-center py-16">
              <section className='card bg-secondary text-secondary-content max-w-[300px]'>
                <div className='card-body'>
                  ...
                  {createGameId === null ? (
                    <button className="btn btn-neutral" onClick={handleCreate}>Create</button>
                  ) : (
                    <div className="w-full">
                      <p className="text-center pb-2 font-bold">Game created!</p>
                      <p className="text-center pb-4">{createGameId}</p>
                      <div className="flex justify-center items-center gap-2">
                        ...
                        <button onClick={() => handleJoin(createGameId)} className="btn btn-neutral grow">Join</button>
                      </div>
                    </div>
                  )
                  }
                  <div className="divider">Join game</div>
                  <input value="" onChange={(e) => setInputGameId(e.target.value)} className="input input-bordered" placeholder="Game ID" type="text" />
